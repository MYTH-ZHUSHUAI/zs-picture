...
public class UserPictureServiceImpl extends ServiceImpl<UserPictureMapper, UserPicture>
    ...
    public PictureVO uploadPictureToUserSpace(MultipartFile file,
        ...
        try {
            ...
            if (isOpen == 1) {
                // 检查公共库是否已存在
                Picture existPublicPicture = pictureService.lambdaQuery().eq(Picture::getMd5, md5).one();
            ...
            log.info("图片 MD5={} 已添加到用户空间，spaceId={}, isOpen={}", md5, req.getSpaceId(), isOpen);
    ...
    private UploadPictureResultVO uploadFileToOSS(MultipartFile file, byte[] fileBytes) {
        try {
            return imageUploadManger.uploadPicture(file);
        } catch (Exception e) {
